...
def data_preperation(f, keras_dformat, percent=100):      #data preperation
...
def ConvertH5toTFRecordPreprocessing(datafile,filenumber,series,datadirectory):
    # read file
    print('Loading Data from .....', datafile)
    f=h5py.File(datafile,'r')
    
    #dataset = GetDataAngleParallel(f)
    dataset = data_preperation(f, 'channels_first')

    f.close()

    print(len(dataset.get('X')))
    print(len(dataset.get('Y')))
    print(len(dataset.get('ecal')))
    #return

    dataset = tf.data.Dataset.from_tensor_slices((dataset.get('X'),dataset.get('Y'),dataset.get('ecal')))#.batch(128)

    ##tf.print(tf.shape(dataset))
    #for f0, f1, f2 in dataset.take(10):
    #    print(f0)
    #    print(f1)
    #    print(f2)
    ...
    #Rewrite file names
    filename = datadirectory + '/EleEscan_'+ str(series) +'_'+ str(filenumber) +'.tfrecords'
...
def RetrieveTFRecordpreprocessing(recorddatapaths, batch_size):
    ...
    parsed_dataset = recorddata.map(_parse_function, num_parallel_calls=tf.data.experimental.AUTOTUNE).cache().shuffle(200000).repeat().batch(batch_size, drop_remainder=True).with_options(options)
...
if __name__ == '__main__':
    ...
    file10 = Files[1]
    Files.remove(file10)
    Files.insert(9, file10)
    file10 = Files[11]
    Files.remove(file10)
    Files.insert(19, file10)
    print ("Found {} files. ".format(len(Files)))
    print(Files)

    #feat = RetrieveTFRecordpreprocessing('/eos/user/r/redacost/EleScanstf/EleEscan_1_1.tfrecords',100)
    #bat = 0
    #for b in feat:
    #    bat += 1
    #    print(b.get('Y'))
    ...
    for f in Files:
        if filenumber == 11:
            ...
            series = 2
        print(filenumber)
        finaldata = ConvertH5toTFRecordPreprocessing(f,filenumber,series,outpath)
        #print(finaldata)
        filenumber += 1
