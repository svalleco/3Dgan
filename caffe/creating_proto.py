...
import sys
caffe_root='/data/gkhattak/caffe/'
...
n.conv5 = L.Convolution(n.resh, num_output=64, kernel_size= [6, 6, 8],  pad=[2, 2, 3], engine=1, weight_filler=dict(type='gaussian', std= 0.0001 ), bias_filler=dict(type='constant', value=0))  # (not working for nd) weight_filler=dict(type='msra') => keras he_uniform
...
n.conv4 = L.Convolution(n.upsmpl5, num_output=6, kernel_size= [6, 5, 8],  pad=[2, 2, 0], engine=1, weight_filler=dict(type='gaussian', std= 0.0001 ), bias_filler=dict(type='constant', value=0))# (not working for nd) weight_filler=dict(type='msra')  => keras he_uniform 
n.relu4 = L.ReLU(n.conv4, negative_slope=0.3, engine=1)
n.bn4 = L.BatchNorm(n.relu4, in_place=True)
n.upsmpl4 = L.Deconvolution(n.bn4, convolution_param=dict(num_output=1, group=1, kernel_size=[4, 4, 5], stride = [2, 2, 3], pad=1, weight_filler=dict(type='gaussian', std= 0.0001 ), bias_filler=dict(type='constant', value=0))) # f = [2, 2, 3]
n.conv3 = L.Convolution(n.upsmpl4, num_output=6, kernel_size= [3, 3, 8],  pad=[1, 0, 3], engine=1, weight_filler=dict(type='gaussian', std= 0.0001 ), bias_filler=dict(type='constant', value=0)) # (not working for nd) weight_filler=dict(type='msra')  => keras he_uniform 
n.relu3 = L.ReLU(n.conv3, negative_slope=0.3, engine=1)
n.conv2 = L.Convolution(n.relu3, num_output=1, kernel_size= [2, 2, 2],pad = [2, 0, 3], engine=1, weight_filler=dict(type='gaussian', std= 0.0001 ), bias_filler=dict(type='constant', value=0))   # (not working for nd) weight_filler=dict(type='xavier')
...
n.TAG = L.Input(shape=dict(dim=[batch_size, ]))        #  class
n.event = L.Input(shape=dict(dim=[batch_size, 1]))      #  If real or gen
n.conv1 = L.Convolution(n.ECAL, num_output=32, kernel_size= 5,  pad=2, engine=1, weight_filler=dict(type='gaussian', std= 0.0001 ), bias_filler=dict(type='constant', value=0))
n.relu1 = L.ReLU(n.conv1, negative_slope=0.3, engine=1)
n.drpout1 = L.Dropout(n.relu1, dropout_ratio= 0.2)
n.conv2 = L.Convolution(n.drpout1, num_output=8, kernel_size= 5,  pad=2, engine=1, weight_filler=dict(type='gaussian', std= 0.0001 ), bias_filler=dict(type='constant', value=0))
...
n.tag_loss = L.SigmoidCrossEntropyLoss(n.tag, n.TAG, loss_weight=0.5)
n.aux = L.InnerProduct(n.drpout4, num_output = 1)
n.aux_loss = L.SigmoidCrossEntropyLoss(n.aux, n.event, loss_weight=0.5)
n.loss = L.Eltwise(n.tag_loss, n.aux_loss, operation=1)  #    1 = sum mode  
...
#make solvers
with open ("solver_template3.prototxt", "r") as myfile:
