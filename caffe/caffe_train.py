...
max_iter = 34900  # maximum number of iterations
...
momentum_hist ={}
for layer in generator.net.params:
  m_w = np.zeros_like(generator.net.params[layer][0].data)
  m_b = np.zeros_like(generator.net.params[layer][1].data)
...
for it in range(start_snapshot,max_iter):
 ...
 # discriminator.net.backward()
  ...
  #discriminator.net.backward()
 # discriminator.rmsprop_update_gpu()
...
# run the discriminator on generated data with opposite labels, to get the gradient for the generator
  ...
  for i in range(2):                                                      
      ...
      discriminator.net.blobs['event'].data[...] =bit_flip( np.ones((batch_size,1), dtype='float32'))
      discriminator.net.forward()
      discr_fake_for_generator_loss = np.copy(discriminator.net.blobs['aux_loss'].data)
      generator_loss = generator_loss + discr_fake_for_generator_loss
      ...
      #for layer in generator.net.layers:
       # for blob in layer.blobs:
        #    blob.data[...] -= lr * blob.dif
      for layer in generator.net.params:
        momentum_hist[layer][0] = momentum_hist[layer][0] * momentum + (generator.net.params[layer][0].diff + weight_decay *
                                                       generator.net.params[layer][0].data) * base_lr * lr_w_mult
        momentum_hist[layer][1] = momentum_hist[layer][1] * momentum + (generator.net.params[layer][1].diff + weight_decay *
                                                       generator.net.params[layer][1].data) * base_lr * lr_b_mult
        generator.net.params[layer][0].data[...] -= momentum_hist[layer][0]
        generator.net.params[layer][1].data[...] -= momentum_hist[layer][1]
        generator.net.params[layer][0].diff[...] *= 0
        generator.net.params[layer][1].diff[...] *= 0
  base_lr = base_lr * np.power(gamma, (np.floor(it / stepsize)))

# add by samson                                                                                                         
#display                                                                      
  if it % display_every == 0 or it == max_iter:
    ...
    print >> sys.stderr, "  generator loss: %f" % (generator_loss)
    start = time.time()
  ...
  #snapshot                                                                     
  if it % snapshot_every == 0 or it == snapshot_at_iter or it == max_iter:
