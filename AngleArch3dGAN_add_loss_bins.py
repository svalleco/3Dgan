...
def count(image):
    ...
    bin2 = K.sum(K.tf.where(K.tf.logical_and(image < 0.1, image > 0.05), K.ones_like(image), K.zeros_like(image)), axis=(1, 2, 3))
    bin3 = K.sum(K.tf.where(K.tf.logical_and(image < 0.05, image > 0.01), K.ones_like(image), K.zeros_like(image)), axis=(1, 2, 3))
    bin4 = K.sum(K.tf.where(K.tf.logical_and(image < 0.01, image > 0.0), K.ones_like(image), K.zeros_like(image)), axis=(1, 2, 3))
