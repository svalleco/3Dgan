...
def generator(latent_size=200, keras_dformat='channels_first'):
    ...
    x0 = tf.keras.layers.Reshape(dim) (x)  #shape after (none, 5,5,5)   
    #print(x.shape)
    
    layers = []
    for l in range(25):
        x = tf.keras.layers.Conv2DTranspose(32, (5,5), strides =(3,3), data_format=keras_dformat, padding="same") (x0)
        ...
        #2.Conv Block
        x = tf.keras.layers.Conv2DTranspose(32, (5,5), strides =(2,2), data_format=keras_dformat, padding="same") (x)
        ...
        x = tf.keras.layers.Dropout(0.2)(x)
        #4.Conv Block
        x = tf.keras.layers.Conv2D(16, (2, 2), data_format=keras_dformat, padding='valid', use_bias=False,  kernel_initializer='he_uniform')(x)
        x = tf.keras.layers.LeakyReLU() (x)
        x = tf.keras.layers.BatchNormalization(axis=axis) (x) 
        x = tf.keras.layers.Dropout(0.2)(x)
        #5.Conv Block
        x = tf.keras.layers.Conv2D(1, (1,1), data_format=keras_dformat, padding='valid',  kernel_initializer='he_uniform')(x)
        x = tf.keras.layers.LeakyReLU() (x)
        x = tf.keras.layers.BatchNormalization(axis=axis) (x) 
        x = tf.keras.layers.Dropout(0.2)(x)
    ...
    #print(x.shape)
    x = tf.concat([layers[l] for l in range(25)], axis=axis)
    #print(x.shape)
    x = tf.keras.layers.Conv2D(25, (5, 5), data_format=keras_dformat, padding='same', use_bias=False,  kernel_initializer='he_uniform')(x)
    x = tf.keras.layers.LeakyReLU() (x)
    x = tf.keras.layers.BatchNormalization(axis=axis) (x) 
    x = tf.keras.layers.Dropout(0.2)(x)
