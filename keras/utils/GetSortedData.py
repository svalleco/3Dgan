...
def ecal_angle_3d_ver2(image):
   ...
   x_ref = K.sum(K.sum(image, axis=(2, 3)) * K.cast(K.expand_dims(K.arange(x_shape), 0), dtype='float32'), axis=1)/sumtot
   y_ref = K.sum(K.sum(image, axis=(1, 3)) * K.cast(K.expand_dims(K.arange(y_shape), 0), dtype='float32'), axis=1)/sumtot
   z_ref = K.sum(K.sum(image, axis=(1, 2)) * K.cast(K.expand_dims(K.arange(z_shape), 0), dtype='float32'), axis=1)/sumtot
...
def main():
   ...
   for p in Particles:
     ...
     pdir = os.path.join(sortedpath, '{}MeasuredThetaEscan'.format(p))
     gan.safe_mkdir(pdir)
