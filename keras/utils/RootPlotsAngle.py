...
def plot_ang_hist(ang1, ang2, out_file, angle, labels, ifpdf=True):
   ...
   for i, key in enumerate(ang1):
     hps.append(ROOT.TH1F("G4" + labels[i],"G4" + labels[i], 50, -1, 1))
     hgs.append(ROOT.TH1F("GAN" + labels[i], "GAN" + labels[i], 50, -1, 1))
     hp= hps[i]
     hg= hgs[i]
     if i== 0:
       hp.SetStats(0)
     ...
     hp.SetTitle("Angle Histogram for {} Eta".format(angle) )
     hg.SetStats(0)
...
def plot_angle_error_hist(ang1, ang2, y, out_file, angle, labels, ifpdf=True):
   c1 = ROOT.TCanvas("c1" ,"" ,200 ,10 ,700 ,500) #make
   c1.SetGrid()
   color = 2
   legend = ROOT.TLegend(.7, .8, .9, .9)
   ...
   for i, key in enumerate(ang1):
     hps.append(ROOT.TH1F("G4" + labels[i],"G4" + labels[i], 20, -1, 2.5))
     hgs.append(ROOT.TH1F("GAN" + labels[i], "GAN" + labels[i], 20, -1, 2.5))
     hp= hps[i]
     hg= hgs[i]
     if i== 0:
       hp.SetStats(0)
       hp.SetTitle("Angle Relative Error Histogram for {} eta".format(angle) )
       hp.GetXaxis().SetTitle("Eta")
       hp.GetYaxis().SetTitle("(Eta_{act} - Eta_{pred})/Eta_{act}")
...
def get_plots_angle(var, labels, plots_dir, energies, angles, aindexes, m, n, ifpdf=True, stest=True):
    ...
    for energy in energies:
       ...
       for a, angle in zip(aindexes, angles):
          ...
          plot_ang_hist(var["angle_act" + str(energy) + "ang_" + str(a)], var["angle_gan" + str(energy) + "ang_" + str(a)] , 
                                 os.path.join(ediscdir, angfile + "ang_" + str(a)), angle, alabels)
          plots+=1
          ...
          plot_angle_error_hist(var["angle_act" + str(energy) + "ang_" + str(a)], var["angle_gan" + str(energy) + "ang_" + str(a)], 
                                var["angle" + str(energy) + "ang_" + str(a)], os.path.join(ediscdir, aerrorfile + "ang_" + str(a)), 
                                 angle, alabels)
