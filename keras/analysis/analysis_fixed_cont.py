####   This script takes a certain number of events from data file. Then generates events at same energies by GAN. It places the events in user specified bins and compare the different quantities through plots. Events are also generated at fixed point equal to center of the bin and compared to other plots. ##
import os, h5py
import numpy as np
import matplotlib
...
#Get weights
gen_weights='params_generator_epoch_028.hdf5'
disc_weights='params_discriminator_epoch_028.hdf5'
filename = 'Gen_cont_' + str(n_jets) + 'events.h5'
datafile = "/afs/cern.ch/work/g/gkhattak/public/Ele_v1_1_2.h5"
plots_dir = 'cont_fixed_plots/'
## Get Full data                                                               
...
for energy in energies:
   ...
   var["events_fixed" + str(energy)] = np.zeros((num_events, 25, 25, 25))
   var["max_pos_fixed_" + str(energy)] = np.zeros((num_events, 3))
   var["sumfixed" + str(energy)] = np.zeros((num_events, 3, 25))
   var["energy_fixed" + str(energy)] = np.zeros((num_events, 1))
   #var["energy_auxfixed" + str(energy)] = np.zeros((num_events, 1))
   #var["isreal_fixed" + str(energy)] =np.zeros((num_events, 1))
...
if get_gen:
   ...
   #generate fixed energy points
   for energy in energies:
      noise = np.random.normal(0, 1, (num_events, latent_space))
      fixed_labels = np.ones((num_events, 1), dtype=np.float)   
      fixed_labels = np.multiply(energy, fixed_labels)  #take sampled labels from actual data
      fixed_labels = fixed_labels/100
      var["energy_fixed" + str(energy)]= fixed_labels
      print fixed_labels[:10]
      generator_in = np.multiply(fixed_labels, noise)
      fixed_images = g.predict(generator_in, verbose=False, batch_size=100)
      var["events_fixed" + str(energy)]= np.squeeze(fixed_images)
      var["isrealf"+ str(energy)], var["aux_outf"+ str(energy)], var["ecal_outf"+ str(energy)] = np.array(d.predict(fixed_images, verbose=False, batch_size=100))
...
for j in range(num_events):
   for energy in energies:
      ...
      if get_gen==0:
         var["max_pos_fixed_" + str(energy)][j] = np.unravel_index(var["events_fixed" + str(energy)][j].argmax(), (25, 25, 25))
         var["sumfixed" + str(energy)][j, 0] = np.sum(var["events_fixed" + str(energy)][j], axis=(1,2))
         var["sumfixed" + str(energy)][j, 1] = np.sum(var["events_fixed" + str(energy)][j], axis=(0,2))
         var["sumfixed" + str(energy)][j, 2] = np.sum(var["events_fixed" + str(energy)][j], axis=(0,1))
...
print "Energy\t\t Events\t\tMaximum Value\t\t Maximum loc\t\t\t Mean\t\t\t Minimum\t\t"
for energy in energies:
   print "%d \t\t%d \t\t%f \t\t%s \t\t%f \t\t%f" %(energy, var["index" +str(energy)], np.amax(var["events_gan" + str(energy)]), str(np.unravel_index(var["events_gan" + str(energy)].argmax(), (var["index" + str(energy)], 25, 25, 25))), np.mean(var["events_gan" + str(energy)]), np.amin(var["events_gan" + str(energy)]))
...
if get_gen==0:
   ...
   print "Energy\t\t Events\t\tMaximum Value\t\t Maximum loc\t\t\t Mean\t\t\t Minimum\t\t"
   for energy in energies:
      print "%d \t\t%d \t\t%f \t\t%s \t\t%f \t\t%f" %(energy, num_events, np.amax(var["events_fixed" + str(energy)]), str(np.unravel_index(var["events_fixed" + str(energy)].argmax(), (num_events, 25, 25, 25))), np.mean(var["events_fixed" + str(energy)]), np.amin(var["events_fixed" + str(energy)]))
...
for energy in energies:
   ...
   if get_gen==0:
      ...
      fig+=1
      plot_error(var["energy_fixed" + str(energy)],  var["aux_outf" + str(energy)], num_events, os.path.join(fixdir, ferrorfile), fig, flabel, pos=0)
      fig+=1
      plot_max(var["max_pos_fixed_" + str(energy)], num_events, os.path.join(fixdir, maxfile), fig, flabel)
      fig+=1
      plot_energy_axis(var["sumfixed" + str(energy)], num_events, os.path.join(fixdir, histfile), fig, flabel)
      fig+=1
      plot_ecal(var["events_fixed" + str(energy)], num_events , os.path.join(fixdir, ecalfile), fig, dlabel)
      fig+=1
...
for energy in energies:
   ...
   if get_gen==0:
      plot_max(var["max_pos_fixed_" + str(energy)], num_events, os.path.join(comdir, maxfile), fig, flabel)
   fig+=1
   ...
   if get_gen==0:
      plot_energy_axis(var["sumfixed" + str(energy)], num_events, os.path.join(comdir, histfile), fig, flabel)
   fig+=1
   ...
   if get_gen==0:
      plot_ecal(var["events_fixed" + str(energy)], num_events, os.path.join(comdir, ecalfile), fig, flabel)
   fig+=1
   if get_gen==0:
      ...
      plot_energy(np.multiply(100, var["aux_outf" + str(energy)]), num_events, os.path.join(comdir, energyfile), fig, flabel, 'red')
      fig+=1
      ...
      plot_real(var["isrealf" + str(energy)], num_events, os.path.join(discdir, realfile), fig, flabel)
      fig+=1
      ...
      plot_error(var["energy_fixed" + str(energy)],  var["aux_outf" + str(energy)], var["index" + str(energy)], os.path.join(discdir, errorfile), fig, glabel, pos=0)
      fig+=1
plt.close("all")
# Plot for all
