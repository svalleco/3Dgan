...
plots_dir = 'readbyfile_8p2p1_epo46/'
## Get Full data                                                               
...
print Y.shape
X[X < 1e-6] = 0
energies=[50, 100, 200, 250, 300, 400, 500]  #Energy bins
...
def plot_max(array, index, out_file, num_fig, plot_label, pos=0):
   ...
   plt.subplot(222)
   plt.title('Y-axis')
   label= plot_label + '\n{:.2f}'.format(np.mean(array[0:index, 1]))+ '({:.2f}'.format(np.std(array[0:index, 1])) 
   plt.hist(array[0:index, 1], bins=bins, histtype='step', label=label, normed=1)
   plt.legend(loc=pos, fontsize='xx-small')
   plt.xlabel('Position')

   plt.subplot(223)
   label= plot_label + '\n{:.2f}'.format(np.mean(array[0:index, 2]))+ '({:.2f})'.format(np.std(array[0:index, 2])) 
   plt.hist(array[0:index, 2], bins=bins, histtype='step', label=label, normed=1)
   plt.legend(loc=pos, fontsize='xx-small')
   plt.xlabel('Position')
   plt.ylabel('Events')
...
def plot_energy(array, index, out_file, num_fig, plot_label, color='blue', pos=0):
   ...
   plt.hist(array[:index], bins=ebins, histtype='step', label=label, color=color, normed=1)
...
def plot_energy_wt_hist(array, index, out_file, num_fig, plot_label, pos=0):
   ### Plot total energy deposition cell by cell along x, y, z axis                                          
   bins = np.arange(0, 25, 1)
   plt.figure(num_fig)
   plt.subplot(221)
...
def plot_energy_skwt_hist(array, index, out_file, num_fig, plot_label, pos=0):
   ### Plot total energy deposition cell by cell along x, y, z axis                                          
   bins = np.arange(0, 25, 1)
   plt.figure(num_fig)
   plt.subplot(221)
...
for energy in energies:
   ...
   # Regression results
   var["Ecal_act" + str(energy)] = (np.multiply(100, var["energy_sampled" + str(energy)][:var["index" + str(energy)]])-(np.multiply(50, totalE_act[:var["index" + str(energy)]])))/(np.multiply(100, var["energy_sampled" + str(energy)][:var["index" + str(energy)]]))
   var["Ecal_gan" + str(energy)] = (np.multiply(100, var["energy_sampled" + str(energy)][:var["index" + str(energy)]])-(np.multiply(50, totalE_gan[:var["index" + str(energy)]])))/(np.multiply(100, var["energy_sampled" + str(energy)][:var["index" + str(energy)]]))
   var["Ecalratio_act" + str(energy)] = (np.multiply(100, var["energy_sampled" + str(energy)][:var["index" + str(energy)]]))/(totalE_act[:var["index" + str(energy)]])
   var["Ecalratio_gan" + str(energy)] = (np.multiply(100, var["energy_sampled" + str(energy)][:var["index" + str(energy)]]))/(totalE_gan[:var["index" + str(energy)]])
...
# Moments Computations   
   ...
   for i in range(m):
      ...
      ECAL_momentY = umath.inner1d(var["sumact" + str(energy)][:var["index" + str(energy)], 1], moments)/totalE_act
   ...
   for i in range(m):
      ...
      ECAL_momentZ = umath.inner1d(var["sumact" + str(energy)][:var["index" + str(energy)], 2], moments)/totalE_act
   ...
   for i in range(m):
      relativeIndices = np.tile(np.arange(ecal_size), (var["index" + str(energy)],1))
      moments = np.power((relativeIndices.transpose()-ECAL_midX).transpose(), i+1)
      ECAL_momentX = umath.inner1d(var["sumgan" + str(energy)][:var["index" + str(energy)], 0], moments)/totalE_gan
      if i==0: ECAL_midX = ECAL_momentX.transpose()
      var["x_gan"+ str(energy)][:var["index" + str(energy)], i]= ECAL_momentX
   for i in range(m):
      relativeIndices = np.tile(np.arange(ecal_size), (var["index" + str(energy)],1))
      moments = np.power((relativeIndices.transpose()-ECAL_midY).transpose(), i+1)
      ECAL_momentY = umath.inner1d(var["sumgan" + str(energy)][:var["index" + str(energy)], 1], moments)/totalE_gan
      if i==0: ECAL_midY = ECAL_momentY.transpose()
      var["y_gan"+ str(energy)][:var["index" + str(energy)], i]= ECAL_momentY
   for i in range(m):
      relativeIndices = np.tile(np.arange(ecal_size), (var["index" + str(energy)],1))
      moments = np.power((relativeIndices.transpose()-ECAL_midZ).transpose(), i+1)
      ECAL_momentZ = umath.inner1d(var["sumgan" + str(energy)][:var["index" + str(energy)], 2], moments)/totalE_gan
      if i==0: ECAL_midZ = ECAL_momentZ.transpose()
...
for energy in energies:
   ...
   plot_energy_wt_hist(var["sumact" + str(energy)], var["index" + str(energy)], os.path.join(actdir, "w_" + histallfile), fig+1, dlabel)
   plot_energy_axis(var["sumact" + str(energy)], var["index" + str(energy)], os.path.join(actdir, histallfile), fig+2, dlabel)
   ...
   plot_energy_wt_hist(var["sumgan" + str(energy)], var["index" + str(energy)], os.path.join(gendir, histallfile), fig+5, glabel)
   plot_energy_axis(var["sumgan" + str(energy)], var["index" + str(energy)], os.path.join(gendir, "w_" + histallfile), fig+6, glabel)
   plot_energy_mean(var["sumgan" + str(energy)], var["index" + str(energy)], os.path.join(gendir, meanallfile), fig+7, glabel)
   ...
   plot_ecal_normalized2(var["Ecal_gan" + str(energy)], var["index" + str(energy)], os.path.join(gendir, Ecalfile), fig+12, energy, 'tab:orange', 1)
...
for energy in energies:
   ...
   fig+=1
   plot_energy_axis_log(var["sumact" + str(energy)], var["index" + str(energy)], os.path.join(comdir, "log_" + histfile), fig, dlabel)
   plot_energy_axis_log(var["sumgan" + str(energy)], var["index" + str(energy)], os.path.join(comdir, "log_" + histfile), fig, glabel)
   fig+=1
   ...
   fig+=1
   plot_energy_wt_hist(var["sumact" + str(energy)], var["index" + str(energy)], os.path.join(comdir, "wt_" + histfile), fig, dlabel)
   plot_energy_wt_hist(var["sumgan" + str(energy)], var["index" + str(energy)], os.path.join(comdir, "wt_" + histfile), fig, glabel)
   fig+=1
