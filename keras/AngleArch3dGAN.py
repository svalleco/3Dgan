...
def discriminator(power=1.0, dformat='channels_last'):
    ...
    x = LeakyReLU()(x)
    x = BatchNormalization(axis=caxis, momentum=0.9, epsilon=1e-6)(x)
    x = Dropout(0.2)(x)
    ...
    ang = Lambda(ecal_angle, arguments={'daxis':caxis})(inv_image) # angle calculation
    ecal = Lambda(ecal_sum, arguments={'daxis':daxis})(inv_image) # sum of energies
    add_loss = Lambda(count, arguments={'daxis':daxis})(inv_image) # loss for bin counts
