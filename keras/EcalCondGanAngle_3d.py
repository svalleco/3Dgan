...
def ecal_angle(image):
    ...
    x_ref = K.tf.where(K.equal(sumtot, 0.0), K.zeros_like(x_ref), x_ref/sumtot)
    y_ref = K.tf.where(K.equal(sumtot, 0.0), K.zeros_like(y_ref), y_ref/sumtot)
    z_ref = K.tf.where(K.equal(sumtot, 0.0), K.zeros_like(z_ref), z_ref/sumtot)
    ...
    m = K.tf.where(K.equal(zproj, 0.0), K.zeros_like(zproj), (y_mid-y_ref)/zproj)
    m = K.tf.where(K.tf.less(z, z_ref),  -1 * m, m)
    #tan = K.tf.where(K.tf.greater(K.abs(m), 10.0),  (math.pi/2.0) * K.ones_like(m), tf.atan(m))
