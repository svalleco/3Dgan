...
num_events=20000
stat = 0
filename = 'results/ecal_ratio_chpi_{}events_{}_{}GeV_bin{}_ep104.pdf'.format(num_events, p[0], p[1], bins)
#c.SetLogx ()
Eprof = TProfile("Eprof", "Sampling fraction vs. Ep;Ep [GeV];#S_{f}", bins, p[0], p[1])
Eprof2 = TProfile("Eprof", "Sampling fraction vs. Ep;Ep [GeV];#S_{f}", bins, p[0], p[1],'s')
...
gweight1 = weightdir + '3dgan_weights_gan_training_Ch_pion/params_generator_epoch_104.hdf5'
...
Eprof2.SetLineColor(color[0])
Eprof2.SetFillColorAlpha(color[0], 0.35)
Eprof.SetMarkerColor(color[0])
#Eprof2.SetMarkerColor(color[0])
...
for i, gweight in enumerate(gweights):
   ...
   Gprof2.append( TProfile("Gprof2" +str(i), "Gprof2" + str(i), bins, p[0], p[1], 's'))
   #Generate events
   ...
   for j in range(num_events):
      ...
      Gprof2[i].Fill(Y[j], GData[j]/Y[j])
   Gprof[i].SetLineColor(color[i + 1])
   Gprof[i].SetMarkerColor(color[i + 1])
   Gprof2[i].SetFillColorAlpha(color[i + 1], 0.35)
   #print('GAN data')
   W2 = Gprof[i].Integral()
