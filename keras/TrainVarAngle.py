...
def GetEcalFit(sampled_energies, mod=0, xscale=1):
    if mod==0:
      ...
      return np.multiply(ratio, sampled_energies) * xscale/50
...
def GanTrain3DAngle(discriminator, generator, datapath, EventsperFile, nEvents, WeightsDir, nb_epochs=30, batch_size=128, latent_size=200, gen_weight=6, aux_weight=0.2, theta_weight=0.1, phi_weight=0.1, ecal_weight=0.1, lr=0.001, rho=0.9, decay=0.0, g_weights='params_generator_epoch_', d_weights='params_generator_epoch_', xscale=1):
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(total_batches):
            ...
            real_batch_loss = discriminator.train_on_batch(image_batch, [BitFlip(np.ones(batch_size)), energy_batch, theta_batch, phi_batch, ecal_batch])
            fake_batch_loss = discriminator.train_on_batch(generated_images, [BitFlip(np.zeros(batch_size)), energy_batch, theta_batch, phi_batch, ecal_batch])
        ...
        generator_test_loss = combined.evaluate(generator_ip,
                                                [trick, aux_y, theta, phi, ecal_ip], verbose=False, batch_size=batch_size)
...
def get_parser_var():
    ...
    #parser.add_argument('--datapath', action='store', type=str, default='/eos/project/d/dshep/LCD/DDHEP/*scan_RandomAngle_*_MERGED/*scan_RandomAngle_*.h5', help='HDF5 files to train from.')
