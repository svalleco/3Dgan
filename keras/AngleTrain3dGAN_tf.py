...
def GetDataAngle(FileSearch, xscale =1, xpower=1, yscale = 100, angscale=1, angtype='theta', thresh=1e-4, daxis=-1):
    ...
    f0=h5py.File(Files[0],'r')
    X_train=np.array(f0.get('ECAL'))* xscale
    Y_train=np.array(f0.get('energy'))/yscale
    ...
    if angtype in f0:
        ang_train = np.array(f0.get(angtype))[indexes]
    else:
        ang_train = gan.measPython(X)
        ang_train = -1.
    ang_train = ang_train.astype(np.float32)
    X_train = np.expand_dims(X_train, axis=daxis)
    ecal_train=ecal_train[indexes]
    ecal_train=np.expand_dims(ecal_train, axis=daxis)
    if xpower !=1.:
      X_train = np.power(X_train, xpower)
    for F in range(1, len(Files)-1):
      print ('Loading Data from .....', F)
      f=h5py.File(Files[F],'r')
      ...
      if angtype in Files[F]:
        ang = np.array(Files[F].get(angtype))[indexes]
      else:
