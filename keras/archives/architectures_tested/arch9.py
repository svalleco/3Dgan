...
def discriminator():
    ...
    x = Conv3D(32, 2, 2, 5, border_mode='same')(image)
    x = LeakyReLU()(x)
    x = Dropout(0.2)(x)

    x = ZeroPadding3D((2, 2,2))(x)
    x = Conv3D(16, 3, 3, 5, border_mode='valid')(x)
    x = LeakyReLU()(x)
    x = BatchNormalization()(x)
    x = Dropout(0.2)(x)

    x = ZeroPadding3D((2, 2, 2))(x)
    x = Conv3D(16, 4, 4, 5, border_mode='valid')(x)
    x = LeakyReLU()(x)
    x = BatchNormalization()(x)
    x = Dropout(0.2)(x)
...
def generator(latent_size=1024, return_intermediate=False):
    ...
    loc = Sequential([
        ...
        ZeroPadding3D((2, 2, 0)),
        Conv3D(16, 5, 5, 8, init='he_uniform'),
        LeakyReLU(),
        BatchNormalization(),
        UpSampling3D(size=(2, 2, 3)),

        ZeroPadding3D((0,0,3)),
        Conv3D(8, 3, 3, 8, init='he_uniform'),
        LeakyReLU(),
