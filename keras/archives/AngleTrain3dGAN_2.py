...
def main():
    ...
    from AngleArch3dGAN_2 import generator, discriminator
...
def hist_count(images, p=1.0, daxis=(1, 2, 3)):
    limits=np.array([0.05, 0.03, 0.02, 0.0125, 0.008, 0.003]) # bin boundaries used
    x = np.power(images, 1./p)
    bin1 = np.sum(np.where(x>(limits[0]) , x, 0.), axis=daxis)
    bin2 = np.sum(np.where((x<(limits[0])) & (x>(limits[1])), x, 0.), axis=daxis)
    bin3 = np.sum(np.where((x<(limits[1])) & (x>(limits[2])), x, 0.), axis=daxis)
    bin4 = np.sum(np.where((x<(limits[2])) & (x>(limits[3])), x, 0.), axis=daxis)
    bin5 = np.sum(np.where((x<(limits[3])) & (x>(limits[4])), x, 0.), axis=daxis)
    bin6 = np.sum(np.where((x<(limits[4])) & (x>(limits[5])), x, 0.), axis=daxis)
    bin7 = np.sum(np.where((x<(limits[5])) & (x>0.), x, 0.), axis=daxis)
    #bin8 = np.sum(np.where(x==0, x, 0.), axis=daxis)
    bins = np.concatenate([bin1, bin2, bin3, bin4, bin5, bin6, bin7], axis=1)
    bins[np.where(bins==0)]=1e-8 # so that an empty bin will be assigned a count of 1 to avoid unstability
    return bins
...
def Gan3DTrainAngle(discriminator, generator, datapath, nEvents, WeightsDir, pklfile, nb_epochs=30, batch_size=128, latent_size=200, loss_weights=[3, 0.1, 25, 0.1, 0.1], lr=0.001, rho=0.9, decay=0.0, g_weights='params_generator_epoch_', d_weights='params_discriminator_epoch_', xscale=1, xpower=1, angscale=1, angtype='theta', yscale=100, thresh=1e-4, analyse=False, resultfile="", energies=[], dformat='channels_last', particle='Ele', verbose=False, warm=False, prev_gweights='', prev_dweights=''):
    ...
    for epoch in range(nb_epochs):
        ...
        while nb_file < len(Trainfiles) and index < nb_train_batches:
            ...
            #out = discriminator.predict(image_batch)
            #print('disc_out={}'.format(np.transpose(out[4][:5])) )
            #print(np.transpose(disc_out[4][:5].astype(int)))
            #print('add_loss_batch={}'.format(np.transpose(add_loss_batch[:5])))
