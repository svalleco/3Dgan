...
disc_weights="veganweights2/params_discriminator_epoch_045.hdf5"
gen_weights= "veganweights2/params_generator_epoch_045.hdf5"
plots_dir = "my_plots/"
latent = 200
num_data = 100000
num_events = 2000
...
def plot_flat_energy(arrayx, arrayy, arrayz, index, out_file, num_fig, plot_label):
...
def plot_energy(array, index, out_file, num_fig, plot_label, color='blue', pos=0):
...
def plot_energy_axis_log(arrayx, arrayy, arrayz, index, out_file, num_fig, plot_label, pos=0):
   ...
   plt.subplot(223)
   sumz_array =np.mean(arrayz)
   label= plot_label + '\n{:.2f}'.format(np.mean(sumz_array))+ '({:.2f})'.format(np.std(sumz_array))
   plt.semilogy( sumz_array/index, label=plot_label)
...
def plot_energy_wt_hist(arrayx, arrayy, arrayz, index, out_file, num_fig, plot_label, pos=0):
   ### Plot total energy deposition cell by cell along x, y, z axis                                          
   bins = np.arange(0, 25, 1)
   plt.figure(num_fig)
   plt.subplot(221)
   plt.title('X-axis')
   sumx_array = arrayx.mean(axis = 0)
   label= plot_label + '\n{:.2f}'.format(np.mean(sumx_array))+ '({:.2f})'.format(np.std(sumx_array))
...
def plot_energy_axis(arrayx, arrayy, arrayz, index, out_file, num_fig, plot_label, pos=0):
   ### Plot total energy deposition cell by cell along x, y, z axis                                                                                                                                                          
   plt.figure(num_fig)
   plt.subplot(221)
   plt.title('X-axis')
...
def plot_energy_mean(arrayx, arrayy, arrayz, index, out_file, num_fig, plot_label):
   ### Plot total energy deposition cell by cell along x, y, z axis                                                   
...
def plot_real(array, index, out_file, num_fig, plot_label):
   ...
   plt.figure(num_fig)
   plt.title('Real/ Fake')
   label= plot_label + '\n{:.2f}'.format(np.mean(array[0:index, 0]))+ '({:.2f})'.format(np.std(array[0:index, 0]))
   plt.hist(array[0:index, 0], bins=bins, histtype='step', label= label, normed=1)
   plt.legend()
   plt.ylabel('Events')
...
def plot_error(array1, array2, index, out_file, num_fig, plot_label, pos=2):
   ...
   plt.hist(np.multiply(100, error), bins=bins, histtype='step', label=label)
...
def plot_ecal_error(array1, array2, index, out_file, num_fig, plot_label, pos=2):
   ...
   plt.figure(num_fig)
   bins = np.linspace(-150, 150, 30)
   parray = np.multiply(100, array2[:index])
   earray = np.sum(array1[:index], axis=(1, 2, 3))
...
def plot_ecal(array, index, out_file, num_fig, plot_label):
   ...
   ecal_array= np.multiply(50, ecal_array[:index])
...
def plot_1tototal(array, index, out_file, num_fig, plot_label):
   ...
   ecal_total=np.sum(array[:index, :, :, :], axis=(1, 2, 3))
...
def plot_moment(array, index, out_file, num_fig, plot_label, moment):
   # plot error                         
   plt.figure(num_fig)
   print 'plotting..{} moment'.format(moment)
...
def plot_e_normalized(array, index, out_file, num_fig, energy, color, bar=0):
   ...
   label= str(energy) 
   if bar==0:
      plt.scatter(energy, np.mean(array[:index]), label=label, color =color)
   else:
      plt.errorbar(energy, np.mean(array[:index]), yerr= np.std(array[:index]), fmt='o', label=label, color =color)
   plt.ylabel('Ep - Er / Ep')
   plt.xlabel('Energy')
...
def plot_e_normalized2(array, index, out_file, num_fig, energy, color, bar=1):
   # plot error                                                                             
   plt.figure(num_fig)
   label= str(energy)                                                                      
   if bar==0:
      ...
      plt.errorbar(energy, np.mean(array[:index]), yerr= np.std(array[0:index]), fmt='o', color =color, elinewidth=1 , capsize=4)
   plt.ylabel('Ep - Er / Ep')
   plt.xlabel('Energy GeV')
   #plt.ylim(ymax = 0.2, ymin = -0.2)
   plt.legend( fontsize='x-small', title="Data = Blue\nGAN= Orange")
   plt.savefig(out_file)

def plot_ecal_normalized(array, index, out_file, num_fig, energy, color, bar=0):
   # plot error                                                                                                                                                                                              
   plt.figure(num_fig)
   label= str(energy)
   if bar==0:
      plt.scatter(energy, np.mean(array[0:index]), label=label, color =color)
   else:
      plt.errorbar(energy, np.mean(array[0:index]), yerr= np.std(array[0:index]), fmt='o', label=label, color =color)
...
def plot_ecal_normalized2(array, index, out_file, num_fig,  energy, color, bar=1):
   # plot error                                                                            
   plt.figure(num_fig)
   label= str(energy)
   if bar==0:
      ...
      plt.errorbar(energy, np.mean(array[:index]), yerr= np.std(array[:index]), fmt='o',  elinewidth=1 , capsize=4, label='Data', color=color)
   plt.ylabel('Ep - Ecal / Ep')
   plt.xlabel('Energy GeV')
...
def get_max(images):
    ...
    for i in range(index):
       ...
       max_pos[i] = np.array(np.unravel_index(images[0].argmax(), (25, 25, 25)))
    return max_pos
...
if __name__ == '__main__':
    ...
    for energy in energies:
      ...
      var["E_act" + str(energy)] = var["energy"+ str(energy)] - var["aux_act" + str(energy)]
      var["E_gan" + str(energy)] = var["energy"+ str(energy)] - var["aux_gan" + str(energy)]
    #### Generate Data table to screen                                                                                 
    print "Actual Data"
    print "Energy\tEvents\tMaximum Value\tMaximum loc\t\tMean\t\tMomentx2\tMomenty2\tMomentz2"
    for energy in energies:
      print "{}\t{}\t{:.4f}\t\t{}\t{:.4f}\t\t{:.4f}\t\t{:.4f}\t\t{:.4f}" .format(energy, var["index" +str(energy)], np.amax(var["events_act" + str(energy)]), np.mean(var["max_pos_act" + str(energy)], axis=0), np.mean(var["events_act" + str(energy)]), np.mean(var["momentX_act"+ str(energy)][:, 1]), np.mean(var["momentY_act"+ str(energy)][:, 1]), np.mean(var["momentZ_act"+ str(energy)][:, 1]))

    #### Generate GAN table to screen                                                                                  
    print "Generated Data"
    print "Energy\tEvents\tMaximum Value\tMaximum loc\t\tMean\t\tMomentx2\tMomenty2\tMomentz2"
    for energy in energies:
      print "{}\t{}\t{:.4f}\t\t{}\t{:.4f}\t\t{:.4f}\t\t{:.4f}\t\t{:.4f}".format(energy, var["index" +str(energy)], np.amax(var["events_gan" + str(energy)]), np.mean(var["max_pos_gan" + str(energy)], axis=0), np.mean(var["events_gan" + str(energy)]), np.mean(var["momentX_gan"+ str(energy)][:, 1]), np.mean(var["momentY_gan"+ str(energy)][:, 1]), np.mean(var["momentZ_gan"+ str(energy)][:, 1]))
...
for energy in energies:
   ...
   plot_max(var["max_pos_act" + str(energy)], var["index" + str(energy)], os.path.join(actdir, maxfile), fig, dlabel)
   fig+=1
   plot_energy_axis(var["sumsx_act"+ str(energy)], var["sumsy_act"+ str(energy)], var["sumsz_act"+ str(energy)], var["index" + str(energy)], os.path.join(actdir, histfile), fig, dlabel)
   fig+=1
   plot_ecal(var["events_act" + str(energy)], var["index" + str(energy)], os.path.join(actdir, ecalfile), fig, dlabel)
   fig+=1
   plot_max(var["max_pos_gan" + str(energy)], var["index" + str(energy)], os.path.join(gendir, maxfile), fig, glabel)
   fig+=1
   plot_energy_axis(var["sumsx_gan"+ str(energy)], var["sumsy_gan"+ str(energy)], var["sumsz_gan"+ str(energy)], var["index" + str(energy)], os.path.join(gendir, histfile), fig, glabel)
   fig+=1
   plot_ecal(var["events_gan" + str(energy)], var["index" + str(energy)], os.path.join(gendir, ecalfile), fig, glabel)
   fig+=1
   plot_ecal_error(var["events_act" + str(energy)], var["energy" + str(energy)], var["index" + str(energy)], os.path.join(actdir, ecalerrorfile), fig, dlabel)
   fig+=1
   plot_ecal_error(var["events_gan" + str(energy)], var["energy" + str(energy)], var["index" + str(energy)], os.path.join(gendir, ecalerrorfile), fig, glabel)
   fig+=1

   plot_energy(np.multiply(100, var["aux_act" + str(energy)]), var["index" + str(energy)], os.path.join(actdir, energyfile), fig, dlabel, 'green')
   fig+=1
   plot_real(var["isreal_act" + str(energy)], var["index" + str(energy)], os.path.join(discdir, drealfile), fig, glabel)
   fig+=1
   plot_error(var["energy" + str(energy)],  var["aux_act" + str(energy)], var["index" + str(energy)], os.path.join(discdir, derrorfile), fig, dlabel, pos=0)
   fig+=1
   plot_energy(np.multiply(100, var["aux_gan" + str(energy)]), var["index" + str(energy)], os.path.join(gendir, energyfile), fig, glabel, 'green')
   fig+=1
   plot_real(var["isreal_gan" + str(energy)], var["index" + str(energy)], os.path.join(discdir, grealfile), fig, glabel)
   fig+=1
   plot_error(var["energy" + str(energy)],  var["aux_gan" + str(energy)], var["index" + str(energy)], os.path.join(discdir, gerrorfile), fig, glabel, pos=0)
   fig+=1
   plt.close("all")
## Make combined plots
for energy in energies:
   ...
   plot_flat_energy(var["sumsx_act"+ str(energy)], var["sumsy_act"+ str(energy)], var["sumsz_act"+ str(energy)], var["index" + str(energy)], os.path.join(actdir, flatfile), fig, dlabel)
   plot_energy_wt_hist(var["sumsx_act"+ str(energy)], var["sumsy_act"+ str(energy)], var["sumsz_act"+ str(energy)], var["index" + str(energy)], os.path.join(actdir, "w_" + histallfile), fig+1, dlabel)
   plot_energy_axis(var["sumsx_act"+ str(energy)], var["sumsy_act"+ str(energy)], var["sumsz_act"+ str(energy)], var["index" + str(energy)], os.path.join(actdir, histallfile), fig+2, dlabel)
   plot_energy_mean(var["sumsx_act"+ str(energy)], var["sumsy_act"+ str(energy)], var["sumsz_act"+ str(energy)], var["index" + str(energy)], os.path.join(actdir, meanallfile), fig+3, dlabel)
   plot_flat_energy(var["sumsx_gan"+ str(energy)], var["sumsy_gan"+ str(energy)], var["sumsz_gan"+ str(energy)], var["index" + str(energy)], os.path.join(gendir, flatfile), fig + 4, glabel)
   plot_energy_wt_hist(var["sumsx_gan"+ str(energy)], var["sumsy_gan"+ str(energy)], var["sumsz_gan"+ str(energy)], var["index" + str(energy)], os.path.join(gendir, histallfile), fig+5, glabel)
   plot_energy_axis(var["sumsx_gan"+ str(energy)], var["sumsy_gan"+ str(energy)], var["sumsz_gan"+ str(energy)], var["index" + str(energy)], os.path.join(gendir, "w_" + histallfile), fig+6, glabel)
   plot_energy_mean(var["sumsx_gan"+ str(energy)], var["sumsy_gan"+ str(energy)], var["sumsz_gan"+ str(energy)], var["index" + str(energy)], os.path.join(gendir, meanallfile), fig+7, glabel)
   plot_e_normalized2(var["E_act" + str(energy)], var["index" + str(energy)], os.path.join(actdir, Efile), fig + 8, energy, 'tab:blue', 1)
   plot_e_normalized2(var["E_gan" + str(energy)], var["index" + str(energy)], os.path.join(gendir, Efile), fig + 9, energy, 'tab:orange', 1)
   plot_e_normalized2(var["E_act" + str(energy)], var["index" + str(energy)], os.path.join(comdir, Efile), fig + 10, energy, 'tab:blue')
   plot_e_normalized2(var["E_gan" + str(energy)], var["index" + str(energy)], os.path.join(comdir, Efile), fig + 10, energy, 'tab:orange')
   plot_ecal_normalized2(var["ecal_act" + str(energy)], var["index" + str(energy)], os.path.join(actdir, Ecalfile), fig+11, energy, 'tab:blue', 1)
   plot_ecal_normalized2(var["ecal_act" + str(energy)], var["index" + str(energy)], os.path.join(gendir, Ecalfile), fig+12, energy, 'tab:orange', 1)
   plot_ecal_normalized2(var["ecal_act" + str(energy)], var["index" + str(energy)], os.path.join(comdir, Ecalfile), fig+13, energy, 'tab:blue')
   plot_ecal_normalized2(var["ecal_gan" + str(energy)], var["index" + str(energy)], os.path.join(comdir, Ecalfile), fig+13, energy, 'tab:orange')
   #plot_ecal_primary(var["Ecalratio_act" + str(energy)], var["index" + str(energy)], os.path.join(actdir, Ecalratiofile), fig+14, energy, 'r')
   #plot_ecal_primary(var["Ecalratio_gan" + str(energy)], var["index" + str(energy)], os.path.join(gendir, Ecalratiofile), fig+15, energy, 'b')
...
for energy in energies:
   ...
   plot_max(var["max_pos_act" + str(energy)], var["index" + str(energy)], os.path.join(comdir, maxfile), fig, dlabel)
   plot_max(var["max_pos_gan" + str(energy)], var["index" + str(energy)], os.path.join(comdir, maxfile), fig, glabel)
   fig+=1
   plot_energy_axis_log(var["sumsx_act" + str(energy)], var["sumsy_act"+ str(energy)], var["sumsz_act"+ str(energy)], var["index" + str(energy)], os.path.join(comdir, "log_" + histfile), fig, dlabel)
   plot_energy_axis_log(var["sumsx_gan" + str(energy)], var["sumsy_gan"+ str(energy)], var["sumsz_gan"+ str(energy)], var["index" + str(energy)], os.path.join(comdir, "log_" + histfile), fig, glabel)
   fig+=1
   plot_energy_axis(var["sumsx_act" + str(energy)], var["sumsy_act"+ str(energy)], var["sumsz_act"+ str(energy)], var["index" + str(energy)], os.path.join(comdir, histfile), fig, dlabel)
   plot_energy_axis(var["sumsx_gan" + str(energy)], var["sumsy_gan"+ str(energy)], var["sumsz_gan"+ str(energy)], var["index" + str(energy)], os.path.join(comdir, histfile), fig, glabel)
   fig+=1
   plot_energy_wt_hist(var["sumsx_act" + str(energy)], var["sumsy_act"+ str(energy)], var["sumsz_act"+ str(energy)], var["index" + str(energy)], os.path.join(comdir, "wt_" + histfile), fig, dlabel)
   plot_energy_wt_hist(var["sumsx_gan" + str(energy)], var["sumsy_gan"+ str(energy)], var["sumsz_gan"+ str(energy)], var["index" + str(energy)], os.path.join(comdir, "wt_" + histfile), fig, glabel)
   fig+=1
  # plot_energy_skwt_hist(var["sumact" + str(energy)], var["index" + str(energy)], os.path.join(comdir, "skwt_" + histfile), fig, dlabel)
  # plot_energy_skwt_hist(var["sumgan" + str(energy)], var["index" + str(energy)], os.path.join(comdir, "skwt_" + histfile), fig, glabel)
  # fig+=1
   ...
   fig+=1
   plot_energy(np.multiply(100, var["aux_act" + str(energy)]), var["index" + str(energy)], os.path.join(comdir, energyfile), fig, dlabel, 'blue')
   plot_energy(np.multiply(100, var["aux_gan" + str(energy)]), var["index" + str(energy)], os.path.join(comdir, energyfile), fig, glabel, 'green')
   fig+=1
   ...
   fig+=1
   plot_error(var["energy" + str(energy)],  var["aux_act" + str(energy)], var["index" + str(energy)], os.path.join(discdir, errorfile), fig, dlabel, pos=0)
   plot_error(var["energy" + str(energy)],  var["aux_gan" + str(energy)], var["index" + str(energy)], os.path.join(discdir, errorfile), fig, glabel, pos=0)
   fig+=1
   ...
   for mmt in range(m):
      ...
      plot_moment(var["momentX_act" + str(energy)][:, mmt], var["index" + str(energy)], os.path.join(mdir, mfile), fig, dlabel, mmt)
      plot_moment(var["momentX_gan" + str(energy)][:, mmt], var["index" + str(energy)], os.path.join(mdir, mfile), fig, glabel, mmt)
      fig+=1
      mfile=yfile + str(mmt) + ".pdf"
      plot_moment(var["momentY_act" + str(energy)][:, mmt], var["index" + str(energy)], os.path.join(mdir, mfile), fig, dlabel, mmt)
      plot_moment(var["momentY_gan" + str(energy)][:, mmt], var["index" + str(energy)], os.path.join(mdir, mfile), fig, glabel, mmt)
      fig+=1
      mfile=zfile + str(mmt) + ".pdf"
      plot_moment(var["momentZ_act" + str(energy)][:, mmt], var["index" + str(energy)], os.path.join(mdir, mfile), fig, dlabel, mmt)
      plot_moment(var["momentZ_gan" + str(energy)][:, mmt], var["index" + str(energy)], os.path.join(mdir, mfile), fig, glabel, mmt)
      fig+=1
