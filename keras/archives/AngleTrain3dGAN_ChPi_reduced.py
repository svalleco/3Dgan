...
def get_parser():
    ...
    parser.add_argument('--name', action='store', type=str, default='gan_training_Ch_pion_reduced2', help='Unique identifier can be set for each training')
    return parser
...
def GetDataAngle2(datafile, xscale =1, xpower=1, yscale = 100, angscale=1, angtype='theta', thresh=1e-4, daxis=-1):
    ...
    Y = Y.astype(np.float32)
    ecal = np.sum(X, axis=(1, 2, 3))
    indexes = np.where((ecal > 10.0) & (Y > 1.5) & (Y < 3.5))
