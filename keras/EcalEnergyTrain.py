...
def GanTrain2D(discriminator, generator, datapath, EventsperFile, nEvents, WeightsDir, mod=0, nb_epochs=30, batch_size=128, latent_size=200, gen_weight=6, aux_weight=0.2, ecal_weight=0.1, lr=0.001, rho=0.9, decay=0.0, g_weights='params_generator_epoch_', d_weights='params_generator_epoch_', xscale=1):
    ...
    for epoch in range(nb_epochs):
        ...
        X_train, Y_train, ecal_train = GetData2D(Trainfiles[0], xscale=xscale)
        print(Y_train[:10])
        nb_file=1
        ...
        for index in range(total_batches):
            ...
            if (loaded_data - used_data) < batch_size + 1 and (nb_file < len(Trainfiles)):
                X_temp, Y_temp, ecal_temp = GetData2D(Trainfiles[nb_file], xscale=xscale)
...
if __name__ == '__main__':
    ...
    GanTrain2D(d, g, datapath, EventsperFile, nEvents, weightdir, mod=fitmod, nb_epochs=nb_epochs, batch_size=batch_size, gen_weight=8, aux_weight=0.2, ecal_weight=0.1, xscale = xscale)
