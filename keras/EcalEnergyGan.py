...
def discriminator(dformat='channels_last'):
    ...
    x = LeakyReLU()(x)
    x = BatchNormalization(axis=baxis, epsilon=1e-6)(x, training=True)
    x = Dropout(0.2)(x)
...
def generator(latent_size=200, return_intermediate=False, dformat='channels_last'):
     K.set_image_data_format(dformat)
     if dformat =='channels_last':
        dim = (7,7,8,8)
        baxis=-1
