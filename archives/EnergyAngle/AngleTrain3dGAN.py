...
def Gan3DTrainAngle(discriminator, generator, datapath, EventsperFile, nEvents, WeightsDir, mod=0, nb_epochs=30, batch_size=128, latent_size=200, gen_weight=6, aux_weight=0.2, ecal_weight=0.1, ang_weight=0.1, lr=0.001, rho=0.9, decay=0.0, g_weights='params_generator_epoch_', d_weights='params_discriminator_epoch_', xscale=1, angscale=1):
    ...
    for index, dtest in enumerate(Testfiles):
       if index == 0:
           X_test, Y_test, ang_test, ecal_test = GetDataAngle(dtest, xscale=xscale, angscale=angscale, thresh=1e-4)
       else:
           if X_test.shape[0] < numTest:
              X_temp, Y_temp, ang_temp, ecal_temp = GetDataAngle(dtest, xscale=xscale, angscale=angscale, thresh=1e-4)
    ...
    for epoch in range(nb_epochs):
        ...
        X_train, Y_train, ang_train, ecal_train = GetDataAngle(Trainfiles[0], xscale=xscale, angscale=angscale, thresh=1e-4)
        #print(ecal_train[:10])
        ...
        for index in range(total_batches):
            ...
            generator_ip = np.column_stack((sampled_energies, sampled_ang, noise))
            #cond = np.expand_dims(cond, axis=2)
            #generator_ip = np.concatenate(cond, noise, axis=0)
            ...
            fake_batch_loss = discriminator.train_on_batch(generated_images, [BitFlip(np.zeros(batch_size)), sampled_energies, sampled_ang, ecal_ip])
            ...
            for _ in range(2):
                ...
                generator_ip = np.column_stack((sampled_energies, sampled_ang, noise))
                #cond = np.expand_dims(cond, axis=2)
                #generator_ip = np.expand_dims(generator_ip, axis=2)
        ...
        noise = np.random.normal(0, 1, (2 * nb_test, latent_size - 2))
    ...
    open('dcgan-history-theta_concat.pkl', 'wb'))
...
if __name__ == '__main__':
    ...
    from EcalCondGan_concat import generator, discriminator
