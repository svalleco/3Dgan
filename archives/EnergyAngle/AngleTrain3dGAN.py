...
import Train_rootfit_scaled as train

#@profile

#This funcions loads data from a file and also does any pre processing
def GetDataAngle(datafile, xscale =1, yscale = 100, thetascale=10, phiscale=1):
    ...
    phi = np.array(f.get('phi'))/phiscale
    X[X < 1e-6] = 0
...
def GanTrain3DAngle(discriminator, generator, datapath, EventsperFile, nEvents, WeightsDir, nb_epochs=30, batch_size=128, latent_size=200, gen_weight=6, aux_weight=0.2, theta_weight=0.1, phi_weight=0.1, ecal_weight=0.1, lr=0.001, rho=0.9, decay=0.0, g_weights='params_generator_epoch_', d_weights='params_generator_epoch_', mod=1, xscale=1):
    ...
    # Getting Data
    Trainfiles, Testfiles = gan.DivideFiles(datapath, nEvents=nEvents, EventsperFile = EventsperFile, Fractions=[.9,.1], datasetnames=["ECAL"], Particles =["Ele"])
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(total_batches):
            if verbose:
                ...
                if index % 100 == 0:
                    print('processed {}/{} batches in {} seconds'.format(index + 1, total_batches, time.time()- epoch_start))
            print('batch = {}'.format(index))
            loaded_data = X_train.shape[0]
            print('loaded_data {}'.format(loaded_data))
            used_data = file_index * batch_size
            print('used data {}'.format(used_data))
            ...
            file_index +=1
            print('loaded image'.format(image_batch.shape[0]))        
            ...
            ecal_ip = gan.GetEcalFit(sampled_energies, particle=particle, mod=mod, xscale=xscale)
            cond = np.column_stack((sampled_energies, sampled_theta, sampled_phi))
            cond = np.expand_dims(cond, axis=2)
            generator_ip = np.multiply(cond, noise)
            generated_images = generator.predict(generator_ip, verbose=0)        
            real_batch_loss = discriminator.train_on_batch(image_batch, [gan.BitFlip(np.ones(batch_size)), energy_batch, theta_batch, phi_batch, ecal_batch])
            fake_batch_loss = discriminator.train_on_batch(generated_images, [gan.BitFlip(np.zeros(batch_size)), sampled_energies, sampled_theta, sampled_phi, ecal_ip])
            ...
            for _ in range(2):
                ...
                gen_losses.append(combined.train_on_batch(
                    [generator_ip],
                    [trick, sampled_energies.reshape((-1, 1)), sampled_theta, sampled_phi, ecal_ip]))
        ...
        generator_test_loss = combined.evaluate(generator_ip,
                                                [trick, sampled_energies, sampled_theta, sampled_phi, ecal_ip], verbose=False, batch_size=batch_size)
...
if __name__ == '__main__':
    ...
    GanTrain3DAngle(d, g, datapath, EventsperFile, nEvents, weightdir, nb_epochs=nb_epochs, batch_size=batch_size, gen_weight=2, aux_weight=0.1, theta_weight=0.1, phi_weight=0.1, ecal_weight=0.1, mod=mod, xscale = xscale)
