...
def GetDataAngleTheta(datafile, xscale =1, yscale = 100, thetascale=1):
...
def Gan3DTrainAngle(discriminator, generator, datapath, EventsperFile, nEvents, WeightsDir, mod=0, nb_epochs=30, batch_size=128, latent_size=200, gen_weight=6, aux_weight=0.2, ecal_weight=0.1, theta_weight=0.1, lr=0.001, rho=0.9, decay=0.0, g_weights='params_generator_epoch_', d_weights='params_discriminator_epoch_', xscale=1, thetascale=1):
    ...
    thetamin, thetamax= (-0.523599) , (0.523599) 
    ...
    discriminator.compile(
        ...
        loss_weights=[gen_weight, aux_weight, theta_weight, ecal_weight]
    )
    ...
    fake, aux, theta, ecal= discriminator(fake_image)
    combined = Model(
        input=[latent],
        output=[fake, aux, theta, ecal],
        name='combined_model'
    )
    combined.compile(
    ...
    for index, dtest in enumerate(Testfiles):
       if index == 0:
           X_test, Y_test, theta_test, ecal_test = GetDataAngleTheta(dtest, xscale=xscale, thetascale=thetascale)
       else:
           if X_test.shape[0] < nEvents * f[1]:
              X_temp, Y_temp, theta_temp, ecal_temp = GetDataAngleTheta(dtest, xscale=xscale, thetascale=thetascale)
    ...
    #print(ecal_test[:10])
    #print(Y_test[:10])
    print('theta test from{} to {}\n{}'.format(np.amin(theta_test), np.amax(theta_test), theta_test[:10]))
    print(X_test.shape)
    print(Y_test.shape)
    #print(Y_test[:10])
    print('*************************************************************************************')
    ...
    for epoch in range(nb_epochs):
        ...
        X_train, Y_train, theta_train, ecal_train = GetDataAngleTheta(Trainfiles[0], xscale=xscale, thetascale=thetascale)
        #print(ecal_train[:10])
        ...
        for index in range(total_batches):
            ...
            if (loaded_data - used_data) < batch_size + 1 and (nb_file < len(Trainfiles)):
                X_temp, Y_temp, theta_temp, ecal_temp = GetDataAngleTheta(Trainfiles[nb_file], xscale=xscale, thetascale=thetascale)
            ...
            #print('theta out = ', discout[2][:5])
            #print(theta_batch.shape)
            #print(discout[2].shape)
            #loss = 100 * np.mean(np.absolute((theta_batch.reshape(-1, 1)-discout[2])/theta_batch))
            #print('Calculated real loss = {}'.format(loss))
            #error = np.mean(np.absolute(theta_batch-discout[2]))
            #print('Calculated real error = {}'.format(error))
            ...
            for _ in range(2):
                ...
                gen_losses.append(combined.train_on_batch(
                    [generator_ip],
                    [trick, sampled_energies.reshape((-1, 1)), sampled_theta, ecal_ip]))
        ...
        discriminator_test_loss = discriminator.evaluate(
            X, [y, aux_y, theta, ecal], verbose=False, batch_size=batch_size)
        ...
        generator_test_loss = combined.evaluate(generator_ip,
                                                [trick, sampled_energies, sampled_theta, ecal_ip], verbose=False, batch_size=batch_size)
