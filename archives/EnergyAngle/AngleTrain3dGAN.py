...
def Gan3DTrainAngle(discriminator, generator, datapath, EventsperFile, nEvents, WeightsDir, mod=0, nb_epochs=30, batch_size=128, latent_size=200, gen_weight=6, aux_weight=0.2, ecal_weight=0.1, ang_weight=0.1, lr=0.001, rho=0.9, decay=0.0, g_weights='params_generator_epoch_', d_weights='params_discriminator_epoch_', xscale=1, angscale=1):
    ...
    discriminator.compile(
        ...
        loss_weights=[gen_weight, ang_weight, ang_weight]
    )
    ...
    fake, ang1, ang2= discriminator(fake_image)
    combined = Model(
        input=[latent],
        output=[fake, ang1, ang2],
        name='combined_model'
    )
    combined.compile(
    ...
    for epoch in range(nb_epochs):
        ...
        while nb_file < len(Trainfiles):
            ...
            noise = np.multiply(ang1_batch.reshape(-1, 1), noise) # Same angle as G4
            ...
            #print(disc_out[0][3])
            real_batch_loss = discriminator.train_on_batch(image_batch, [BitFlip(np.ones(batch_size)), ang1_batch, ang2_batch])
            fake_batch_loss = discriminator.train_on_batch(generated_images, [BitFlip(np.zeros(batch_size)), ang1_batch, ang2_batch])
            ...
            for _ in range(2):
                noise = np.random.normal(0, 1, (batch_size, latent_size))
                noise = np.multiply(ang1_batch.reshape(-1, 1), noise)
                generator_ip = noise
                gen_losses.append(combined.train_on_batch(
                    [generator_ip],
                    [trick, ang1_batch, ang2_batch]))
        ...
        noise = np.multiply(ang1_test.reshape(-1, 1), noise)
        generator_ip = noise
        ...
        discriminator_test_loss = discriminator.evaluate( X, [y, ang1, ang2], verbose=False, batch_size=batch_size)
        discriminator_train_loss = np.mean(np.array(epoch_disc_loss), axis=0)

        noise = np.random.normal(0, 1, (2 * nb_test, latent_size))
        noise = np.multiply(ang1.reshape(-1, 1), noise)
        generator_ip = noise
        trick = np.ones(2 * nb_test)
        generator_test_loss = combined.evaluate(generator_ip,
                [trick, ang1, ang2], verbose=False, batch_size=batch_size)
        generator_train_loss = np.mean(np.array(epoch_gen_loss), axis=0)
        train_history['generator'].append(generator_train_loss)
        train_history['discriminator'].append(discriminator_train_loss)
        test_history['generator'].append(generator_test_loss)
        test_history['discriminator'].append(discriminator_test_loss)
        print('{0:<20s} | {1:6s} | {2:12s} | {3:12s}| {4:5s} '.format(
            'component', *discriminator.metrics_names))
        print('-' * 65)
        ROW_FMT = '{0:<20s} | {1:<4.2f} | {2:<10.2f} | {3:<10.2f}| {4:<10.2f} '
    ...
    open('dcgan-history-angle-only.pkl', 'wb'))
...
if __name__ == '__main__':
    ...
    from EcalCondGan_2loss_angle import generator, discriminator
    ...
    weightdir = 'thetaweights_angle_only'
