...
def main():
    ...
    plotsdir = 'results/angle_optimization_gan_training_z_shapes_1d' # plot directory
    particle = "Ele" 
    scale = 1
    threshold = 0
    ang = 1
    ...
    PlotResultsRoot(result, plotsdir, start=0, stop=len(gen_weights), epochs=epoch, fits=fits, ang=ang)
...
def PlotResultsRoot(result, resultdir, start, stop, epochs, fits, ang=1):
    c =[]
    color=2
    x_size= len(result[0])
    minr = 100 * np.ones(x_size)
    l=len(epochs)
    ep=np.zeros((l))
    res= np.zeros((l))
    minr_n = np.zeros(x_size)
    for x in np.arange(x_size):
      c.append(ROOT.TCanvas("c"+str(x) ,"" ,200 ,10 ,700 ,500))
      c[x].SetGrid ()
      legend = ROOT.TLegend(.5, .6, .9, .9)
      for i, epoch in enumerate(epochs):
        if result[i][x]< minr[x]:
           minr[x] = result[i][x]
           minr_n[x] = epoch
        ep[i]=epoch
        res[i]=result[i][x]
      gw  = ROOT.TGraph(l , ep, res )
      gw.SetLineColor(color)
      legend.AddEntry(gw, "Gromov Wass for {} z = {:.4f} (epoch {})".format(x, minr[x], minr_n[x]), "l")
      gw.SetTitle("Optimization function for z position {}: Gromov Wasserstein;Epochs;loss".format(x))
      gw.Draw('ALP')
      c[x].Update()
      legend.Draw()
      c[x].Update()
      c[x].Print(os.path.join(resultdir, "result_{}.pdf".format(x)))
      fits = ['pol2']
      for i, fit in enumerate(fits):
        gw.Fit(fit)
        gw.GetFunction(fit).SetLineColor(color)
        gw.GetFunction(fit).SetLineStyle(2)
        if i == 0:
          legend.AddEntry(gw.GetFunction(fit), 'fit', "l")
      
        c[x].Update()
        c[x].Print(os.path.join(resultdir, "result_{}.pdf".format(fit)))
        print ('The plots are saved to {}'.format(resultdir))
...
def metric(var, energies, m, angtype='mtheta', x=25, y=25, z=25, ang=1):
   for i, energy in enumerate([energies[0]]):
     metric_act = var["sumsz_act" + str(energy)]
     metric_gan = var["sumsz_gan"+ str(energy)]
     result = []
     z_size = metric_act.shape[1]
     for j in np.arange(z_size):
         result.append(wass(np.squeeze(metric_act[:, j]), np.squeeze(metric_gan[:, j])))
     print('result', result)
   return result
