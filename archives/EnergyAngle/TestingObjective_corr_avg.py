...
def main():
    ...
    plotsdir = 'results/angle_optimization_gan_training_corr_avg' # plot directory
    particle = "Ele" 
    scale = 1
    threshold = 0
    ang = 1
...
def PlotResultsRoot(result, resultdir, start, epochs, fits, ang=1):
    ...
    legend.AddEntry(gw, "Corr loss = {:.4f} (epoch {})".format(minr, minr_n), "l")
    gw.SetTitle("Optimization function: Corr loss;Epochs;loss")
    gw.Draw('ALP')
    c1.Update()
    legend.Draw()
    c1.Update()
...
def GetResults(metric, resultdir, gen_weights, g, datapath, sorted_path, particle="Ele", scale=100, power=1, thresh=1e-6, ang=1, concat=1, preproc=preproc, postproc=postproc):
    ...
    for i in range(len(gen_weights)):
       ...
       if i==0:
           ...
           cumsum = np.sum(np.array(result))
           avg.append(cumsum/(i + 1))
       else:
           cumsum = np.sum(np.array(result[i-5:]))
           avg.append(cumsum/5)
    ...
    for i in range(len(gen_weights)):                                                                                            
       ...
       print (result[i])
       print ('The moving average for ......',gen_weights[i])
       print (avg[i])
...
def metric(var, energies, m, angtype='mtheta', x=25, y=25, z=25, ang=1):
   ...
   for i, energy in enumerate([energies[0]]):
     ...
     #else:
         #moment=np.vstack((moment, np.hstack((var["momentX_act" + str(energy)], var["momentY_act" + str(energy)], var["momentZ_act" + str(energy)]))))
         #shapes = np.vstack((shapes, np.hstack((var["sumsx_act"+ str(energy)], var["sumsy_act"+ str(energy)], var["sumsz_act"+ str(energy)])) ))
         #sampfr = np.vstack((sampfr, np.reshape(var["sf_act"+ str(energy)], (1, -1))))
         #if ang:angles = np.vstack((angles, np.reshape(var["mtheta_act" + str(energy)], (-1, 1))))
     ...
     max_act = np.reshape(np.max(metric_act, axis=0), (1, -1))
     min_act = np.reshape(np.min(metric_act, axis=0), (1, -1))
     metric_a = np.where(max_act>0, np.divide((metric_act - min_act), (max_act-min_act)), metric_act)
     max_gan = np.reshape(np.max(metric_gan, axis=0), (1, -1))
     min_gan = np.reshape(np.min(metric_gan, axis=0), (1, -1))
     metric_g = np.where(max_gan>0, np.divide((metric_gan- min_gan), (max_gan- min_gan)), metric_gan)
     """
     ...
     #p = ot.unif(metric_a.shape[0])
     #q = ot.unif(metric_g.shape[0])
     ...
     #C1 = norm(C1, np.amin(C1), np.amax(C1))
     #C2 = norm(C2,  np.amin(C2),np.amax(C2))
     #print('normalizing')
     #C1 = C1 + 0.5
     #C2 = C2 + 0.5
     ...
     C2 = C2/np.amax(C2)
     w = np.meshgrid(p, q)
     res = np.sum(w * np.absolute(C1-C2))
