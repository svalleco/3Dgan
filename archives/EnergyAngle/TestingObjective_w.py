...
def main():
    ...
    from AngleArch3dGAN_newarch3 import generator # architecture
    ...
    plotsdir = 'results/angle_optimization_newarch3_lr_wass' # plot directory
    particle = "Ele" 
    scale = 1
    threshold = 0
    ang = 1
...
def PlotResultsRoot(result, resultdir, start, epochs, fits, ang=1):
    ...
    legend1.AddEntry(ge, "Energy error min = {:.4f} (epoch {})".format(mine, mine_n), "l")
    mg.Add(ge)
    legend2.AddEntry(ge, "Energy error min = {:.4f} (epoch {})".format(mine, mine_n), "l")
    mg2.Add(ge)
    ...
    mg.Add(gs)
    legend1.AddEntry(gs, "Sampling Fr. error  = {:.4f} (epoch {})".format(mins, mins_n), "l")
    mg2.Add(gs)
    legend2.AddEntry(gs, "Sampling Fr. error  = {:.4f} (epoch {})".format(mins, mins_n), "l")
    c1.Update()
    ...
    c2.cd()
    mg2.SetTitle("Optimization function: Mean Relative Error on shower shapes and sampling fraction;Epochs;Error")
    mg2.Draw('ALP')
    c2.Update()
    legend2.Draw()
    c2.Update()
