...
def GetDataAngle(datafile, power=0.5, xscale =1, yscale = 100, angscale=1, angtype='theta', thresh=1e-4):
...
def Gan3DTrainAngle(discriminator, generator, datapath, EventsperFile, nEvents, WeightsDir, pklfile, mod=0, nb_epochs=30, batch_size=128, latent_size=200, gen_weight=6, aux_weight=0.2, ecal_weight=0.1, ang_weight=10, lr=0.001, rho=0.9, decay=0.0, g_weights='params_generator_epoch_', d_weights='params_discriminator_epoch_', xscale=1, angscale=1, angtype='theta', yscale=100, thresh=1e-4, power=0.5):
    ...
    for index, dtest in enumerate(Testfiles):
       if index == 0:
           X_test, Y_test, ang_test, ecal_test = GetDataAngle(dtest, power=power, xscale=xscale, angscale=angscale, angtype=angtype, thresh=thresh)
       else:
           if X_test.shape[0] < numTest:
              X_temp, Y_temp, ang_temp,  ecal_temp = GetDataAngle(dtest, power=power, xscale=xscale, angscale=angscale, angtype=angtype, thresh=thresh)
    ...
    for epoch in range(nb_epochs):
        ...
        X_train, Y_train, ang_train, ecal_train = GetDataAngle(Trainfiles[0], power=power, xscale=xscale, angscale=angscale, angtype=angtype, thresh=thresh)
        nb_file=1
        ...
        while nb_file < len(Trainfiles):
            ...
            if (loaded_data - used_data) < (batch_size + 1 ):
                X_temp, Y_temp, ang_temp, ecal_temp = GetDataAngle(Trainfiles[nb_file], power=power, xscale=xscale, angscale=angscale, angtype=angtype, thresh=thresh)
                nb_file+=1
...
if __name__ == '__main__':
    ...
    from AngleArch3dGAN_p75 import generator, discriminator
    ...
    weightdir = 'weights/3Dweights_p25' # renamed to keep record
    #pklfile = params.pklfile
    pklfile = 'results/3dgan_history_p25.pkl'
    ...
    angtype = 'mtheta'
    power = 0.25 # required for preprocessing
    ...
    Gan3DTrainAngle(d, g, datapath, EventsperFile, nEvents, weightdir, pklfile, mod=fitmod, nb_epochs=nb_epochs, batch_size=batch_size,
                    ...
                    xscale = xscale, angscale=ascale, yscale=yscale, thresh=thresh, angtype=angtype, power=power)
