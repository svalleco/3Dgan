...
def count(image):
    ...
    bin1 = K.sum(K.tf.where(image > 0.15, K.ones_like(image), K.zeros_like(image)), axis=(1, 2, 3))
    bin2 = K.sum(K.tf.where(K.tf.logical_and(image < 0.15, image > 0.1), K.ones_like(image), K.zeros_like(image)), axis=(1, 2, 3))
    bin3 = K.sum(K.tf.where(K.tf.logical_and(image < 0.1, image > 0.05), K.ones_like(image), K.zeros_like(image)), axis=(1, 2, 3))
    bin4 = K.sum(K.tf.where(K.tf.logical_and(image < 0.05, image > 0.0), K.ones_like(image), K.zeros_like(image)), axis=(1, 2, 3))
