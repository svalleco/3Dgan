...
def main():
    ...
    if keras.__version__ == '1.2.2':
        from AngleArch3dGAN_newarch import generator, discriminator
    else:
    ...
    nEvents = params.nbEvents#events to use in training
    ascale = params.ascale#scaling factor for angle
    yscale = params.yscale#scaling factor for energy
    xscale = params.xscale#scaling factor for ecal
    xpower = params.xpower#power of angle
    analyse=params.analyse # if analysing
    energies =params.energies # Bins for analysis
...
def get_parser():
    ...
    parser.add_argument('--datapath', action='store', type=str, default='/data/shared/gkhattak/*Measured4ThetaEscan/*.h5', help='HDF5 files to train from.')
    parser.add_argument('--nbEvents', action='store', type=int, default=200000, help='Total Number of events used for Training')
    parser.add_argument('--verbose', action='store', default=False, help='Whether or not to use a progress bar')
    parser.add_argument('--xscale', action='store', type=int, default=None, help='Multiplication factor for ecal deposition')
    ...
    parser.add_argument('--ascale', action='store', type=int, default=None, help='Multiplication factor for angle input')
    ...
    parser.add_argument('--gen_weight', action='store', default=3, help='loss weight for generation real/fake loss')
    parser.add_argument('--aux_weight', action='store', default=0.1, help='loss weight for auxilliary energy regression loss')
    parser.add_argument('--ang_weight', action='store', default=25, help='loss weight for angle loss')
    parser.add_argument('--ecal_weight', action='store', default=0.1, help='loss weight for ecal sum loss')
    parser.add_argument('--hist_weight', action='store', default=0.1, help='loss weight for additional bin count loss')
    parser.add_argument('--thresh', action='store', default=None, help='Threshold for cell energies')
    ...
    parser.add_argument('--name', action='store', type=str, default='clean', help='Identifier for training.')
    return parser
...
def GetDataAngle(datafile, xscale =None, xpower=None, yscale = 100, angscale=None, angtype='theta', thresh=None):
    print ('Loading Data from .....', datafile)
    f=h5py.File(datafile,'r')
    ang = np.array(f.get(angtype))
    X=np.array(f.get('ECAL'))
    if xscale: X=X* xscale
    Y=np.array(f.get('energy'))/yscale
    bins = np.array(f.get('bins'))
    if thresh:  X[X < thresh] = 0
    X = X.astype(np.float32)
    Y = Y.astype(np.float32)
    ang = ang.astype(np.float32)
    X = np.expand_dims(X, axis=-1)
    ecal = np.sum(X, axis=(1, 2, 3))
    if xpower: X = np.power(X, xpower)
    return X, Y, ang, ecal, bins
...
def Gan3DTrainAngle(discriminator, generator, datapath, nEvents, WeightsDir, pklfile, nb_epochs=30, batch_size=128, latent_size=200, loss_weights=[3, 0.1, 25, 0.1, 0.1], lr=0.001, rho=0.9, decay=0.0, g_weights='params_generator_epoch_', d_weights='params_discriminator_epoch_', xscale=1, xpower=1, angscale=1, angtype='theta', yscale=100, thresh=1e-4, analyse=False, resultfile="", energies=[]):
    ...
    for epoch in range(nb_epochs):
        ...
        X_train, Y_train, ang_train, ecal_train, bins_train = GetDataAngle(Trainfiles[0], xscale=xscale, xpower=xpower, angscale=angscale, angtype=angtype, thresh=thresh)
        nb_file=1
        epoch_gen_loss = []
        epoch_disc_loss = []
        index = 0
        total_batches = 0
        file_index=0
        #repeat till batches are processed
        while nb_file < len(Trainfiles) and index < nb_Train:
            ...
            loaded_data = X_train.shape[0] #events loaded
            used_data = file_index * batch_size #events processed
            if (loaded_data - used_data) < (batch_size + 1 ): #if all loaded processed
                #remove processed
                ...
                bins_train = bins_train[(file_index * batch_size):]
                #read new data in temp                                                
                X_temp, Y_temp, ang_temp, ecal_temp, bins_temp = GetDataAngle(Trainfiles[nb_file], xscale=xscale, xpower=xpower, angscale=angscale, angtype=angtype, thresh=thresh)
                nb_file+=1
                #concatenate left over data to new data
                X_train = np.concatenate((X_train, X_temp))
                Y_train = np.concatenate((Y_train, Y_temp))
                ang_train = np.concatenate((ang_train, ang_temp))
                ecal_train = np.concatenate((ecal_train, ecal_temp))
                bins_train = np.concatenate((bins_train, bins_temp))
            ...
            add_loss_batch = np.expand_dims(bins_train[(file_index * batch_size):(file_index + 1) * batch_size], axis=-1)
            file_index +=1
            #generate fake image
        ...
        X_test, Y_test, ang_test, ecal_test, bins_test = GetDataAngle(Testfiles[0], xscale=xscale, xpower=xpower, angscale=angscale, angtype=angtype, thresh=thresh)
        nb_batches = int(X_test.shape[0] / batch_size) #batches loaded
        disc_test_loss=[]
        gen_test_loss =[]
        nb_file=1
        index=0
        file_index=0
        while ((nb_file < len(Testfiles)) or (nb_batches > 0)) and index < nb_test_batches: #till all batches in test files are porcessed
           if nb_batches==0:# if less than one batch left
               #remove all processed batches
               ...
               bins_test = bins_test[(file_index * batch_size):]
               #read new data to temp arrays
               X_temp, Y_temp, ang_temp, ecal_temp, bins_temp= GetDataAngle(Testfiles[nb_file], xscale=xscale, xpower=xpower, angscale=angscale, angtype=angtype, thresh=thresh)
               nb_file+=1
               # concatenate to events left from last file
               X_test = np.concatenate((X_test, X_temp))
               Y_test = np.concatenate((Y_test, Y_temp))
               ang_test = np.concatenate((ang_test, ang_temp))
               ecal_test = np.concatenate((ecal_test, ecal_temp))
               bins_test = np.concatenate((bins_test, bins_temp))
           ...
           add_loss_batch = bins_test[(file_index * batch_size):(file_index + 1) * batch_size]
           add_loss_batch = np.expand_dims(add_loss_batch, axis=-1)
           file_index +=1
           nb_batches -=1
           #generate fake batch with same energy and angles
           noise = np.random.normal(0, 1, (batch_size, latent_size-1))
           noise = np.multiply(energy_batch.reshape(-1, 1), noise)
           generator_ip = np.concatenate((ang_batch.reshape(-1, 1), noise), axis=1)
