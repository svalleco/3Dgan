...
def main():
   ...
   var= perform_calculations_multi(g, d, gweights, dweights, energies, datapath, sortdir, gendir, discdir, num_data=nbEvents
            , num_events=binevents, m=moments, scales=xscales, thresh=thresh, flags=flags, latent=latent, particle=particle)
...
def get_parser():
    ...
    parser.add_argument('--plotsdir', action='store', type=str, default='results/Analysis_plots_fixed_thresh0/', help='Directory to store the analysis plots.')
    ...
    parser.add_argument('--cell', action='store', type=int, default=1, help='Whether to plot cell energies..0)Not plotted...1)Only for bin with uniform spectrum.....2)For all energy bins')
    ...
    parser.add_argument('--gweights', action='store', type=str, default=['params_generator_epoch_041.hdf5'], help='list for paths to Generator weights.')
    parser.add_argument('--dweights', action='store', type=str, default=['params_discriminator_epoch_041.hdf5'], help='list for paths to Discriminator weights')
    ...
    parser.add_argument('--energies', action='store', type=int, default=[0], help='Energy bins for analysis')
