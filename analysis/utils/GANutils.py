...
def get_sorted_angle(datafiles, energies, angles, aindexes, flag=False, num_events1=10000, num_events2=2000, tolerance1=5, tolerance2=0.05, Data=GetAngleData, angtype='theta', thresh=1e-6, offset=0.0):
    srt = {}
    for index, datafile in enumerate(datafiles):
...
def save_sorted(srt, energies, aindexes, srtdir, ang=0):
    safe_mkdir(srtdir)
    for energy in energies:
...
def save_discriminated(disc, energy, discdir, nloss=3, ang=0):
    safe_mkdir(discdir)
...
def get_disc(energy, discdir, nloss=3, ang=0):
    filename = os.path.join(discdir, "Disc_{:03d}.hdf5".format(energy))
...
def generate(g, index, energy_labels, angle_labels, latent=256, concat=1, ang=0):
...
def perform_calculations_angle(g, d, gweights, dweights, energies, angles, aindexes, datapath, sortdir, gendirs, discdirs, num_data, num_events, m, xscales, angscales, flags, latent, events_per_file=10000, particle='Ele', Data=GetAngleData, angtype='eta', thresh=1e-6, offset=0.0, nloss=3, concat=1):
    ...
    if read_data: # Read from sorted dir
       start = time.time()
       var = load_sorted(sortedpath, energies, ang)
       ...
       if save_data:
          save_sorted(var, energies, sortdir, ang)
    total = 0
    for energy in energies:
    ...
    for gen_weights, disc_weights, scale, ascale, i in zip(gweights, dweights, xscales, angscales, np.arange(len(gweights))):
       ...
       for energy in energies:
          ...
          if read_gen:
             ...
             start = time.time()
             var["events_gan" + str(energy)]['n_'+ str(i)] = generate(g, var["index" + str(energy)], var["energy" + str(energy)]/100, (var["angle"+ str(energy)]) * ascale, latent, concat, ang)
             if save_gen:
          ...
          if read_disc:
             disc_out = get_disc(energy, discdir, ang)
             ...
             start = time.time()
             if nloss==4:
                 var["isreal_act" + str(energy)]['n_'+ str(i)], var["aux_act" + str(energy)]['n_'+ str(i)], var["angle_act"+ str(energy)]['n_'+ str(i)], var["angle2_act"+ str(energy)]['n_'+ str(i)], var["ecal_act"+ str(energy)]['n_'+ str(i)]= discriminate(d, var["events_act" + str(energy)] * scale)
             ...
             if save_disc:
               ...
               save_discriminated(discout, energy, discdir, ang)
