...
def main():
    ...
    plotsdir = 'results/angle_optimization_gan_training_gw_entropic_kl' # plot directory
    particle = "Ele" 
    scale = 1
    threshold = 0
    ang = 1
...
def PlotResultsRoot(result, resultdir, start, epochs, fits, ang=1):
    ...
    legend.AddEntry(gw, "Min  = {:.4f} (epoch {})".format(minr, minr_n), "l")
    gw.SetTitle("Optimization function: Entropic Gromov Wasserstein (kl);Epochs;loss")
    gw.Draw('ALP')
    c1.Update()
    legend.Draw()
    c1.Update()
