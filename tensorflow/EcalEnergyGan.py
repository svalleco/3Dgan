...
from tensorflow.python.keras.layers import (Input, Dense, Reshape, Flatten, Lambda,
                          Dropout, BatchNormalization, Activation, Embedding)
from tensorflow.python.keras._impl.keras.layers.advanced_activations import LeakyReLU
from tensorflow.python.keras._impl.keras.layers.convolutional import (UpSampling3D, Conv3D, ZeroPadding3D,
                                        AveragePooling3D)
...
def discriminator(keras_dformat='channels_last'):
    ...
    Model(image, [fake, aux, ecal]).summary()
    return Model(image, [fake, aux, ecal])
...
def generator(latent_size=200, return_intermediate=False, keras_dformat='channels_last'):
     ...
     Model([latent], fake_image).summary()
