...
#v3_4: includes energies below 100GeV
...
def generator(latent_size=200, return_intermediate=False, 
    ...
    x = tf.keras.layers.Conv2D(64, (6, 6), data_format=keras_dformat, padding='valid', use_bias=False,  kernel_initializer='he_uniform')(x)
    x = tf.keras.layers.LeakyReLU() (x)
    x = tf.keras.layers.BatchNormalization(axis=axis) (x) 
    x = tf.keras.layers.Dropout(0.2)(x)
    
    #path1
    x = tf.keras.layers.Conv2D(64, (9, 9), data_format=keras_dformat, padding='same', use_bias=False,  kernel_initializer='he_uniform')(x)
    x = tf.keras.layers.LeakyReLU() (x)
    x = tf.keras.layers.BatchNormalization(axis=axis) (x) 
    x = tf.keras.layers.Dropout(0.2)(x)
    
    x = tf.keras.layers.Conv2D(64, (7, 7), data_format=keras_dformat, padding='same', use_bias=False,  kernel_initializer='he_uniform')(x)
    x = tf.keras.layers.LeakyReLU() (x)
    x = tf.keras.layers.BatchNormalization(axis=axis) (x) 
    x = tf.keras.layers.Dropout(0.2)(x)
    ...
    x = tf.keras.layers.Conv2D(25, (4, 4), data_format=keras_dformat, padding='same', use_bias=False,  kernel_initializer='he_uniform')(x)
    x = tf.keras.layers.LeakyReLU() (x)
    x = tf.keras.layers.BatchNormalization(axis=axis) (x) 
    x = tf.keras.layers.Dropout(0.2)(x)
    p1 = x
    ...
    x = tf.keras.layers.Dropout(0.2)(x)
    p2 = x
    x = tf.keras.layers.Add()([p1, p2])
    
    #path3
    x = tf.keras.layers.Permute([2,3,1])(x)
    #increase channel at z-dimension
    x = tf.keras.layers.Conv2D(64, (9, 9), data_format=keras_dformat, padding='same', use_bias=False,  kernel_initializer='he_uniform')(x)
    x = tf.keras.layers.LeakyReLU() (x)
    x = tf.keras.layers.BatchNormalization(axis=axis) (x) 
    x = tf.keras.layers.Dropout(0.2)(x)
    ...
    p3 = x
    x = tf.keras.layers.Add()([p1, p2, p3])
...
def discriminator(keras_dformat='channels_first'):
    ...
    #dshape=(25, 25, 25, 1)
    inp = tf.keras.layers.Input(shape=dshape)     #Input Image
    x = tf.keras.layers.Reshape([25,25,25])(inp)
    #image = x
    x1 = x
    ...
    ecal = tf.keras.layers.Lambda(lambda x: tf.math.reduce_sum(x, axis=daxis))(inp)    #Energie, die im Netzwerk steckt, Summe über gesamtes NEtzwerk
    

    return tf.keras.Model(inputs=inp, outputs=[fake, aux, ecal])
...
def disc_loss(generator, discriminator,image_batch, energy_batch, ecal_batch, batch_size, label, latent_size=200, wtf=6.0, wa=0.2, we=0.1):
