...
def main():
   ...
   yvar = gan.sort([x, y], energies, num_events=2000)
   thetavar = gan.sort([x, theta], thetas, num_events=2000, tolerance=2)
   phivar = gan.sort([x, phi], phis, num_events=2000, tolerance=2)
...
def PlotAngleCut(events, ang, out_file):
   ...
   hy.Draw()
   hy.GetXaxis().SetTitle("X axis")
   hy.GetXaxis().SetTitle("Z axis")
   canvas.Update()
   canvas.cd(3)
   hz.Draw()
   hz.GetXaxis().SetTitle("X axis")
   hz.GetXaxis().SetTitle("Y axis")
   canvas.Update()
   canvas.Print(out_file)
