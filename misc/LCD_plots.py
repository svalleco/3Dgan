...
def main():
    ...
    plotsdir = "results/LCD_paper_plots_w50/"
    safe_mkdir(plotsdir)
    #Parameters
    ...
    for energy in energies:
        ...
        for i, theta in enumerate(thetas):
           print(var["ecal_act"+ str(energy) + "ang_" + str(i)]) 
           for key in var["ecal_act"+ str(energy) + "ang_" + str(i)]:
           ...
           ecal_act.append(var["ecal_act"+ str(energy) + "ang_" + str(i)])
           ecal_gan.append(var["ecal_gan"+ str(energy) + "ang_" + str(i)])
           energy_act.append(var["energy"+ str(energy) + "ang_" + str(i)])                                         
        PlotEnergyHistY(sumy_act, sumy_gan, os.path.join(plotsdir, 'angle_histy3_{}_0'.format(energy)), energy, thetas)
        PlotEnergyHistY(sumy_act, sumy_gan, os.path.join(plotsdir, 'angle_histy3_log{}_0'.format(energy)), energy, thetas, log=1)
        #plot_ecal_ratio_profile(ecal_act, ecal_gan, energy_act, thetas, os.path.join(plotsdir, 'ecal_{}'.format(energy)), p=[100, 200], ifpdf=True)
...
def PlotEnergyHistY2(sumy_act, sumy_gan, out_file, energy, thetas, log=0, ifC=False):
    canvas = ROOT.TCanvas("canvas" ,"abc" ,200 ,10 ,700 ,500) #make
    canvas.SetGrid()
    label = "Weighted Histograms for {} GeV".format(energy)
    ...
    for i, theta in enumerate(thetas):
        ...
        h1y[i]=my.normalize(h1y[i], 1)
        h2y[i]=my.normalize(h2y[i], 1)
        h1y[i].GetXaxis().SetTitle("Energy deposition along Y axis for {} degrees".format(int(theta)))
        h1y[i].GetYaxis().SetTitle("count/integral")
        h1y[i].GetYaxis().CenterTitle()
...
def plot_ecal_ratio_profile(ecal1, ecal2, y, thetas, out_file, p=[100, 200], ifpdf=True):
    ...
    Eprofs=[]
    Gprofs=[]
    legend = ROOT.TLegend(.1, .8, .9, .9)
    for i, theta in enumerate(thetas):
        canvas.cd(i+1)
        Eprofs.append(ROOT.TProfile("Eprof" + str(i), "Eprof" + str(i), 100, int(p[0]), int(p[1])))
        Gprofs.append(ROOT.TProfile("Gprof" + str(i), "Gprof" + str(i), 100, int(p[0]), int(p[1])))
        Eprof=Eprofs[i]
        Eprof.SetStats(ROOT.kFALSE)
        my.fill_profile(Eprof, ecal1[i]/y[i], y[i])
        Eprof.GetXaxis().SetTitle("Ep GeV for theta={} degrees".format(int(theta)))
        ...
        my.fill_profile(Gprof, ecal2[i]/y[i], y[i])
