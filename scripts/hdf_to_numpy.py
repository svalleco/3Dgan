...
# reverse of the resize() function which pads from 51 to 64 and then blurs to size (to preserve dimensions)
# this function takes in a size image, increases resolution to 64, and then follows previous pad pattern to crop to 51
def restore_pic(numpy_image, size, channel_included=True, format_zxy=True, channels_first=True):
    restored_image = numpy_image
    
    if channel_included:
        if channels_first:   #usual for our pgan tensors 
            restored_image = restored_image[:,0,:,:,:]
        else:   # channels_last
            restored_image = restored_image[:,:,:,:,0]
    
    if format_zxy:   # pgan uses [z, x, y] format
        restored_image = np.moveaxis(restored_image, 0, -1)   # move z back: [x,y,z] 

    # resize XY-plane to (64,64,32)
    if size != 64:
        xy_restored_image = np.zeros((64,64,size/2))   # create an empty 3d_image to store changes
        for z_index in np.arange(int(size/2)):    # index through the size/2 calorimeter layers of the z-axis
            img2d = restored_image[:, :, z_index]   # grab a 2d image from the xy plane
            resized_img2d = cv2.resize(img2d, dsize=(64,64), interpolation=cv2.INTER_NEAREST)
            xy_restored_image[:, :, z_index] = resized_img2d   # save our resized_img2d in the img3d corresponding to the calorimeter layer

        # resize YZ-plane to (size x size/2)
        restored_image = np.zeros((size, size, int(size/2)))   # create an empty 3d_image to store changes            
        for x_index in np.arange(size):    # index through the x-axis
            img2d = xy_restored_image[x_index, :, :] # grab an x image
            resized_img2d = cv2.resize(img2d, dsize=(int(size/2), size), interpolation=cv2.INTER_NEAREST)
            restored_image[x_index, :, :] = resized_img2d   # save our resized_img2d in the img3d corresponding to the x layer
    
    # matrix dimensions: [64, 64, 32] --> [51, 51, 25]
