...
def get_parser():
    ...
    parser.add_argument('--weightsdir', action='store', type=str, default='weights/3dgan_weights_bce_inv', help='Directory to store weights.')
    parser.add_argument('--pklfile', action='store', type=str, default='results/3dgan_history_bce_inv.pkl', help='Pickle file to store losses.')
    ...
    parser.add_argument('--resultfile', action='store', type=str, default='results/3dgan_analysis_bce_inv.pkl', help='File to save losses.')
    ...
    parser.add_argument('--lossweights', action='store', type=int, default=[3, 0.1, 40, 0.1, 10], help='loss weights =[gen_weight, aux_weight, ang_weight, ecal_weight, add loss weight]')
...
def safe_log10(x):
    out = 1.0 * x
    out = K.tf.where(out>0., K.log(out)/K.log(10.), K.zeros_like(out))
    return out
...
def mapping(x):
    ...
    p8 = -1.08072e-02
    res= 1.0 * x
    res= K.tf.where(res<1e-7, K.zeros_like(res), res)
    ...
    val = ((1.0+K.pow(K.abs((log10x-p1)/p2),2.0*p3))/p0) * 1/ ((p4+x*(p7+x*p8))* K.sin(p5*(log10x-p6)))
    return val

def inv_mapping(x):
    out = K.tf.where(x<1e-7, K.zeros_like(x), mapping(x))
    return out

def Gan3DTrainAngle(discriminator, generator, datapath, nEvents, WeightsDir, pklfile, nb_epochs=30, batch_size=128, latent_size=200, loss_weights=[3, 0.1, 25, 0.1, 100], lr=0.001, rho=0.9, decay=0.0, g_weights='params_generator_epoch_', d_weights='params_discriminator_epoch_', xscale=1, xpower=1, angscale=1, angtype='theta', yscale=100, thresh=1e-4, analyse=False, resultfile="", energies=[]):
    ...
    def weighted_kl(y_true, y_pred):
        weighted_true = mapping(y_true) #/26951.5
        y_true = y_true #/11.65
        y_pred = y_pred #/11.65
        y_true = K.clip(y_true, K.epsilon(), None)
        y_pred = K.clip(y_pred, K.epsilon(), None)
        return K.sum(  weighted_true * y_true * K.log(y_true/ y_pred), axis=-1)

    print('[INFO] Building discriminator')
    #discriminator.summary()
    discriminator.compile(
        optimizer=RMSprop(),
        loss=['binary_crossentropy', 'mean_absolute_percentage_error', 'mae', 'mean_absolute_percentage_error', weighted_kl],
    ...
    for epoch in range(nb_epochs):
        ...
        while nb_file < len(Trainfiles) and index < nb_Train:
            ...
            add_loss_batch = normalize(image_batch)
            file_index +=1
        ...
        add_loss_test = normalize(X_test)
