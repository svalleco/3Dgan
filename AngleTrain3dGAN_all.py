...
def get_parser():
    ...
    parser.add_argument('--datapath', action='store', type=str, default='/bigdata/shared/LCDLargeWindow/LCDLargeWindow/varangle/*Escan/*.h5')
    ...
    parser.add_argument('--weightsdir', action='store', type=str, default='weights/3dgan_weights_all', help='Directory to store weights.')
    parser.add_argument('--pklfile', action='store', type=str, default='results/3dgan_history_all.pkl', help='Pickle file to store losses.')
    ...
    parser.add_argument('--resultfile', action='store', type=str, default='results/3dgan_analysis_all.pkl', help='File to save losses.')
    ...
    parser.add_argument('--angtype', action='store', type=str, default='theta', help='Angle to use for Training. It can be theta, mtheta or eta')
    return parser
...
def Gan3DTrainAngle(discriminator, generator, datapath, nEvents, WeightsDir, pklfile, nb_epochs=30, batch_size=128, latent_size=200, loss_weights=[3, 0.1, 25, 0.1, 0.1], lr=0.001, rho=0.9, decay=0.0, g_weights='params_generator_epoch_', d_weights='params_discriminator_epoch_', xscale=1, xpower=1, angscale=1, angtype='theta', yscale=100, thresh=1e-4, analyse=False, resultfile="", energies=[]):
    ...
    generator.load_weights('weights/3dgan_weights_bins_pow_p85/params_generator_epoch_059.hdf5')
    discriminator.load_weights('weights/3dgan_weights_bins_pow_p85/params_discriminator_epoch_059.hdf5')
    for epoch in range(nb_epochs):
        ...
        file_index=0
        print ( nb_Train) 
        while nb_file < len(Trainfiles) and index < total_batches:
            if verbose:
